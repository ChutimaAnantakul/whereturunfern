...
//lodergrapghl
const deletedEnvironmentEventMutations = loader(
  "../../../graphql/mutations/deletedEnvironmentEvent.gql"
);
...
const DeletedEnvironmentEvent = ({ match, history }) => {
  ...
  const [deletedEnvironmentEvent] = useMutation(
    deletedEnvironmentEventMutations
  );
  const idRef = useRef();
  ...
  const handleSubmit = async (e) => {
    if (idRef.current.value === "") {
      ...
      const { data } = await deletedEnvironmentEvent({
        variables: {
          id,
        },
      });
                                  ...
                                  .map((environmentevent) => (
                                    ...
                                    >
                                      <TableCell align="left">
                                        <div>
                                          <h6>
                                            {environmentevent.event.eventnameTh}
                                          </h6>
                                        </div>
                                      </TableCell>
                                      <TableCell>
                                      ...
                                      <TableCell align="center">
                                        {/* <Link
                                          to={`/editcategoryevent/${environmentevent.id}`}
                                        > */}
                                        <Button
                                          variant="contained"
                                          color="inherit"
              ...
              >
                <form>
                  ...
                  <Box p={2}>
                    ...
                    <h6>You want to delete this environmentevent !!</h6> <br />
                    ...
                    <select
                      ...
                      value={data.environmentevent.id}
                      ref={idRef}
                      required
                      disabled
                    >
                      <option
                        key={data.environmentevent.id}
                        value={data.environmentevent.id}
                      >
...
export default DeletedEnvironmentEvent;
